...
db.define_table("Produits_chimiques___Conseils_de_prudence",Field("Produits_chimiques",type="integer"),Field("Conseils_de_prudence",type="integer"),primarykey=["Produits_chimiques","Conseils_de_prudence"])
def boo0(value,row,db):
    rows = db((db.Produits_chimiques___Conseils_de_prudence.Produits_chimiques == row.id)&(db.Produits_chimiques___Conseils_de_prudence.Conseils_de_prudence == db.Conseils_de_prudence.Codes)).select(db.Conseils_de_prudence.ALL)
    t=["w2p_odd odd","w2p_even even"]
    return TABLE(*[TR(r.Conseils_de_prudence, _class=t[idx%2]) for idx,r in enumerate(rows)])
def getDb():
    from os import path
